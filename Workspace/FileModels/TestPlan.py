...
class DataModel(GenericModel.GenericModel):
    ...
    def reset(self):
        ...
        """
        self.testplan = { 'testplan': { 'testfile': [], '@testfile': [] }, '@testplan': {'id': '0' } }
    ...
    def getSorted(self):
        ...
        """
        # to avoid bad behaviour, remove orphan test (the parent does not exist)
        self.fixOrphan()
    ...
    def updateTestFile(self, itemId, testName, testExtension, testDescriptions):
        """
        ...
        for i in xrange(len(testSuites)):
            if testSuites[i]['id'] == str(itemId):
                ...
                for newParam in toAdd:
                    testSuites[i]['properties']['inputs-parameters']['parameter'].append( newParam )
                ...
                for inputDict in properties['outputs-parameters']['parameter']:
                    ...
                    if not newInput:
                        toAdd.append( inputDict )
                for newParam in toAdd:
                    testSuites[i]['properties']['outputs-parameters']['parameter'].append( newParam )
    ...
    def toXml (self):
        ...
        try:
            ...
            # BEGIN NEW in 2.0.0
            self.fixPyXML( data = self.properties['properties']['descriptions'], key = 'description' )
            # END NEW in 2.0.0
            for ts in self.testplan['testplan']['testfile']:
                if sys.version_info < (3,): # python3 support
                    ...
                    # issue Issue 258:
                self.fixPyXML( data = ts['properties']['descriptions'], key = 'description' )
                self.fixPyXML( data = ts['properties']['inputs-parameters'], key = 'parameter' )
                self.fixPyXML( data = ts['properties']['outputs-parameters'], key = 'parameter' )
            ...
            if sys.version_info > (3,): # python3 support
                ...
                xmlDataList.append( bytes2str(self.codecD2X.parseDict( dico = self.testplan )) )
            else:
    ...
    def onLoad(self, decompressedData):
        ...
        try:
            ...
            try:
                ...
                if sys.version_info > (3,):  # python3 support
                    if type(testplan) == bytes: testplan = ''
                ...
                # BEGIN NEW in 5.1.0 :
                if not 'testdevelopment' in ret['file']:
                    self.testdev = time.time()
                else:
                    if sys.version_info > (3,):  # python3 support
                ...
                self.error( "TestPlan > extract properties, testplan %s" % str(e) )
            else:
                try:
                    # BEGIN NEW in 2.0.0: description and can be missing model file 
                    tpl_def = { 'description': [  { 'key': 'author', 'value': '' }, { 'key':  'date', 'value':  '' },
                                                    { 'key': 'summary', 'value': '' }, {'key': 'prerequisites', 'value': ''} ] }
                    if not 'descriptions' in properties: properties['descriptions'] = tpl_def

                    if not len(testplan): testplan = {'testfile': [], '@testfile': [] }

                    if isinstance( testplan['testfile'], dict ): testplan['testfile'] = [ testplan['testfile'] ]
                    
                    for i in xrange(len(testplan['testfile'])):
                        if not 'descriptions' in testplan['testfile'][i]['properties']:
                    ...
                    if 'descriptions' in properties:
                        ...
                        for kv in properties['descriptions']['description']:
                            if kv['key'] == 'prerequisites': foundPrerequis = True
                            if kv['key'] == 'requirement': foundRequirement = True
                            if kv['key'] == 'comments': foundComments = True
                            if kv['key'] == 'libraries': foundLibraries = True
                            if kv['key'] == 'adapters': foundAdapters = True
                            if kv['key'] == 'date': creationDate = kv['value']
                            if kv['key'] == 'state': foundState = True
                            if kv['key'] == 'name': foundTestname = True
                                
                        if not foundPrerequis:
                            properties['descriptions']['description'].append( {'key': 'prerequisites', 'value': ''} )
                        if not foundRequirement:
                            properties['descriptions']['description'].append( {'key': 'requirement', 'value': 'REQ_01'} )
                        if not foundComments:
                            properties['descriptions']['description'].append( {'key': 'comments', 'value': { 'comments': { 'comment': [] } } } )
                                
                        if not foundLibraries:
                            properties['descriptions']['description'].append( {'key': 'libraries', 'value': self.defLibrary} )
                        if not foundAdapters:
                            properties['descriptions']['description'].append( {'key': 'adapters', 'value': self.defAdapter} )
                        if not foundState:
                            properties['descriptions']['description'].append( {'key': 'state', 'value': 'Writing'} )
                        if creationDate is not None:
                            properties['descriptions']['description'].insert( 1, {'key': 'creation date', 'value': creationDate } )
                        if not foundTestname:
                            properties['descriptions']['description'].append( {'key': 'name', 'value': self.testName} )
                            
                    # END NEW in 13.0.0
                    
                    # to keep the compatibility
                    if not 'outputs-parameters' in properties:
                        properties['outputs-parameters'] = { 'parameter': copy.deepcopy(DEFAULT_OUTPUTS) }

                    if not 'inputs-parameters' in properties:
                        properties['inputs-parameters'] = properties['parameters']
                        properties.pop( 'parameters' )

                    if not 'agents' in properties:
                        properties['agents'] = { 'agent': copy.deepcopy(DEFAULT_AGENTS) }
                    ...
                    if isinstance( properties['agents']['agent'], dict):
                         properties['agents']['agent'] = [  properties['agents']['agent'] ]
                    for agt in properties['agents']['agent']:
                        if 'type' not in agt:
                            agt.update( {'type': ''} )
                    # END NEW in 9.0.0 
                except Exception as e:
                    self.error( "TestPlan > fix backward compatibility: %s" % str(e) )
                else:
                    try:
                        if type(testplan) == str or type(testplan) == bytes: # python3 support
                        ...
                        for ts in testplan['testfile']:
                            ...
                            if 'extension' in ts:
                                if ts['extension'] == 'tux':
                                    ...
                                    if not foundTestname:
                                        ts['properties']['descriptions']['description'].append( {'key': 'name', 'value': 'TESTCASE' } )
                                    if not foundRequirement:
                                        ts['properties']['descriptions']['description'].append( {'key': 'requirement', 'value': 'REQ_01' } )

                            if not 'alias' in ts: ts['alias'] = ''
                            if not 'color' in ts: ts['color'] = ''

                            # BEGIN NEW in 6.0.0 : output-parameters  can be missing in the model file, to keep the compatibility
                            if not 'agents' in ts['properties']:
                                ts['properties']['agents'] = { 'agent': copy.deepcopy(DEFAULT_AGENTS) }
                            # END NEW in 6.0.0
                            
                            # bug fix in 10.1
                            if ts['properties']['agents'] == '':  ts['properties']['agents'] = { 'agent': [], '@agent':[] }
                            
                            # BEGIN NEW in 9.0.0 :
                            if isinstance( ts['properties']['agents']['agent'], dict):
                                 ts['properties']['agents']['agent'] = [  ts['properties']['agents']['agent'] ]
                            for agt in ts['properties']['agents']['agent']:
                                if 'type' not in agt:
                                    agt.update( {'type': ''} )
                            # END NEW in 9.0.0 

                            # BEGIN NEW in 5.1.0 : output-parameters  can be missing in the model file, to keep the compatibility
                            if not 'outputs-parameters' in ts['properties']:
                                ts['properties']['outputs-parameters'] = { 'parameter': copy.deepcopy(DEFAULT_OUTPUTS) }
                            # END NEW in 5.1.0
                            # BEGIN NEW in 5.1.0 replace parameters by intput-parameters in the model file, to keep the compatibility
                            if not 'inputs-parameters' in ts['properties']:
                                ts['properties']['inputs-parameters'] = ts['properties']['parameters']
                                ts['properties'].pop( 'parameters' )
                            # END NEW in 5.1.0
                            
                            # NEW in 17.0.0
                            if not 'parent-condition' in ts: ts['parent-condition'] = IF_OK
                            if not 'control' in ts: ts['control'] = ""
                            # END OF NEW
                            
                            if type(ts['properties']['probes']) == str or type(ts['properties']['probes']) == bytes: # python3 support
                                ts['properties']['probes'] = { 'probe':[], '@probe':[] }

                            if type(ts['properties']['inputs-parameters']) == str or type(ts['properties']['inputs-parameters']) == bytes: # python3 support
                                ts['properties']['inputs-parameters'] = { 'parameter':[], '@parameter':[] }

                            if type(ts['properties']['outputs-parameters']) == str or type(ts['properties']['outputs-parameters']) == bytes: # python3 support
                                ts['properties']['outputs-parameters'] = { 'parameter':[], '@parameter':[] }

                            if type(ts['properties']['agents']) == str or  type(ts['properties']['agents']) == bytes: # python3 support
                                ts['properties']['agents'] = { 'agent':[], '@agent':[] }

                            self.fixXML( data = ts['properties']['probes'], key = 'probe' )
                            if '@probe' in ts['properties']['probes']:
                                self.fixXML( data = ts['properties']['probes'], key = '@probe' )

                            self.fixXML( data = ts['properties']['agents'], key = 'agent' )
                            if '@agent' in ts['properties']['agents']:
                                self.fixXML( data = ts['properties']['agents'], key = '@agent' )

                            self.fixXML( data = ts['properties']['inputs-parameters'], key = 'parameter' )
                            ...
                            if sys.version_info < (3,): # python3 support
                                ...
                                self.fixParameterstoUTF8(val=ts['properties']['agents']['agent'])
                                self.fixDescriptionstoUTF8(val=ts['properties']['descriptions']['description'])     

                        self.properties = { 'properties':  properties }
                        if sys.version_info < (3,): # python3 support
                            self.fixParameterstoUTF8(val=self.properties['properties']['inputs-parameters']['parameter'])
                            self.fixParameterstoUTF8(val=self.properties['properties']['outputs-parameters']['parameter'])
                            self.fixParameterstoUTF8(val=self.properties['properties']['agents']['agent'])
                            self.fixDescriptionstoUTF8(val=self.properties['properties']['descriptions']['description'])
                        self.testplan = { 'testplan': testplan, '@testplan': {'id': '0' } }
