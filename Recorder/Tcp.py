...
WINDOW_TITLE="Capture TCP"

class DTcpReplay(QtHelper.EnhancedQDialog, Logger.ClassLogger):
    """
    Http replay dialog
    """
    def __init__(self, parent = None, offlineMode=False):
        ...
        @type parent:
        """
        super(DTcpReplay, self).__init__(parent)
    ...
    def searchTCP(self):
        """
        Search TCP module in assistant
        """
        # modules accessor
        ret = "SutAdapters"
    ...
    def exportToTest(self, TS=True, TU=False):
        ...
        adps = """self.ADP_TCP = %s.TCP.Client(parent=self, bindIp=input('BIND_IP'), bindPort=input('BIND_PORT'), destinationIp=input('DEST_IP'), destinationPort=input('DEST_PORT'), debug=input('DEBUG'), separatorDisabled=True)""" % self.searchTCP()
        ...
        for i in xrange(len(self.requests)):
            ...
            if sentrecv == 'sent':
                steps.append( 'self.step%s = self.addStep(expected="tcp data sent", description="send tcp data", summary="send tcp data")' % j )
            else:
                steps.append( 'self.step%s = self.addStep(expected="tcp data received", description="received tcp data", summary="received tcp data")' % j )
        ...
        for i in xrange(len(self.requests)):
            ...
            if sentrecv == 'sent':
                ...
                tests.append( 'SentMsg = self.ADP_TCP.sendData(data=rawSent)' ) 
                tests.append( 'if not SentMsg:' ) 
                tests.append( '\tself.step%s.setFailed(actual="unable to send data")' % j) 
                tests.append( 'else:' ) 
                tests.append( '\tself.step%s.setPassed(actual="tcp data sent succesfully")' % j ) 
                tests.append( '' ) 
            ...
            if sentrecv == 'recv':
                ...
                tests.append( 'RecvMsg = self.ADP_TCP.hasReceivedData(data=rawRecv, timeout=input("TIMEOUT"))' ) 
                tests.append( 'if RecvMsg is None:' ) 
                tests.append( '\tself.step%s.setFailed(actual="unable to received data")' % j) 
                tests.append( 'else:' ) 
                tests.append( '\tself.step%s.setPassed(actual="tcp data received succesfully")' % j ) 
                tests.append( '' ) 
        if TS:
            init = """self.ADP_TCP.connect()
		connected = self.ADP_TCP.isConnected( timeout=input('TIMEOUT') )
		if not connected:
...
""" % str(self.portEdit.text())
        ...
        if TS:  
            cleanup = """self.ADP_TCP.disconnect()
		disconnected = self.ADP_TCP.isDisconnected( timeout=input('TIMEOUT') )
		if not disconnected:
...
""" % str(self.portEdit.text())
        ...
        self.newTest = self.newTest.replace( "<<PURPOSE>>", 'self.setPurpose(purpose="Replay TCP")' )
    ...
    def readFilePacket(self, pcapFile):
        ...
        self.addLogSuccess(txt="<< Number of TCP packets detected: %s" % len(self.requests))
        ...
        if self.requests:
            ...
            self.addLogWarning( "<< No tcp extracted!" )